...
def make_instance(team,pid,folder,team_folder):
   ...
   if not exists(dest):
       ...
       with open(join(dest,"config.php"), "w") as f:
           f.write( newText )
   dest2 = join(dest,'workspace','apache1')
   if exists(dest2):
      return 10
   # copy folder
   ...
   with open(join(dest,"data","projects.php")) as f:
       ...
       jsonstr.append({'path':'apache1','name':'apache1'})
   ...
   with open(join(dest,"components","ctf","projects.php")) as f:
       ...
       print("foo",jsonstr)
       jsonstr = json.loads(jsonstr)   
       jsonstr.append({'zip-name':"submit_"+pid+".tar",'pid':pid,'folder':folder,'result':pid+'_rsp.txt','project':'apache1'})
       jsonstr = json.dumps(jsonstr)
